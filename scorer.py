...
class XContestScorer:
    def __init__(self, tracklog_or_df, scoring_rules: Dict):
        """
        Initialize the XContest scoring calculator.
    ...
    def _detect_takeoff_point(self):
        ...
        for i in range(len(speeds) - CONSECUTIVE_POINTS):
            ...
            window_hs_products = height_speed_products[i:i + CONSECUTIVE_POINTS]

            avg_window_speed = sum(window_speeds) / CONSECUTIVE_POINTS
            avg_window_hs_product = sum(window_hs_products) / CONSECUTIVE_POINTS

            logger.info(f"Fallback check at index {i}: window_speed={avg_window_speed:.2f}, hs_product={avg_window_hs_product:.2f}")
        ...
        logger.info("Paraglider takeoff not detected, defaulting to first point")
        return 0
        ...
        if len(self.tracklog) > 1000:
            ...
            for i in range(len(tracklog_subset)):
                for j in range(i + 1, len(tracklog_subset)):
                    ...
                    distance = self.calculate_distance(p1.lat, p1.lon, p2.lat, p2.lon)
    ...
    def score_all_triangle_types(self) -> List[Dict]:
        ...
        multipliers = {
            triangle_type: self.scoring_rules.get(triangle_type, {}).get('multiplier', 1.0)
            for triangle_type in best_scores.keys()
        }
        
               
        """
        VIT should first calculate the longest triangle, then check if a longer FAI triangle exists. 
        ...
        for triangle_type, tp_indices in best_triangle_indices.items():             
            ...
            # Find the finish point once    
            #finish_idx = self._find_closest_to_start_idx(tp3_idx)   #VIT ECCOCIIII ##############
            #finish_point = self.tracklog[finish_idx]
            #start_idx = self._find_closest_to_end_idx(tp1_idx, finish_point)             
            #start_point = self.tracklog[start_idx]
            
            start_idx, finish_idx = self._find_start_idx_finish_idx(tp1_idx,tp3_idx)
            start_point = self.tracklog[start_idx]
            finish_point = self.tracklog[finish_idx]
        

            # Calculate additional distances once
            start_to_tp1 = self.calculate_distance(start_point.lat, start_point.lon, tp1.lat, tp1.lon)
            tp3_to_finish = self.calculate_distance(tp3.lat, tp3.lon, finish_point.lat, finish_point.lon)
            ...
            for triangle_type, best_score_data in best_scores.items():
                ...
                if score > best_score_data['score']:
                    ...
                    best_scores[triangle_type] = {'score': score, 'data': triangle_result}
    ...
    def score_free_distance_flight(self, scoring_rules: Dict) -> Dict:
        ...
        for tp_indices in itertools.combinations(hull_tracklog_indices, 3):            
            ...
            start_to_tp1 = self.calculate_distance(self.first_point.lat, self.first_point.lon, tp1.lat, tp1.lon)
            leg1 = self.calculate_distance(tp1.lat, tp1.lon, tp2.lat, tp2.lon)
            leg2 = self.calculate_distance(tp2.lat, tp2.lon, tp3.lat, tp3.lon)
    ...
    def _find_start_idx_finish_idx(self, tp1_idx: int, tp3_idx: int) -> tuple:
        ...
        start_points_rad = np.radians(np.array([(point.lat, point.lon) for point in subset_start]))
        end_points_rad = np.radians(np.array([(point.lat, point.lon) for point in subset_end]))
    ...
    def _find_closest_to_start_idx(self, tp3_idx: int) -> int:
        ...
        points_rad = np.radians(np.array([(point.lat, point.lon) for point in subset]))
...
def process_igc_file(file_path: str, scoring_rules: Dict) -> Dict:
    ...
    # #VIT TEST ##################################################################
    # for point in tracklog[:10]:  # Mostra solo i primi 10 punti
    #     print(f"Lat: {point.lat}, Lon: {point.lon}, Alt: {point.alt}, Time: {point.time}")
    # ############################################################################
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if 'max_distance_info' in result:
            ...
            p1 = max_info['point1']
            p2 = max_info['point2']
            print(f"Point 1: Lat {p1['lat']:.6f}, Lon {p1['lon']:.6f}, Time {p1['time']}")
            print(f"Point 2: Lat {p2['lat']:.6f}, Lon {p2['lon']:.6f}, Time {p2['time']}")
            if max_info['time_difference'] is not None:
                ...
                print(f"Time difference: {int(hours)}h {int(minutes)}m {int(seconds)}s")

        if result.get('out_return'):
            print(f"\nOut distance: {result['out_return']['out_distance']:.2f} km")
            print(f"Missing distance: {result['out_return']['missing_distance']:.2f} km")
            furthest = result['out_return']['furthest_point']
            print(f"Furthest point: Lat {furthest['lat']:.6f}, Lon {furthest['lon']:.6f}, Alt {furthest.get('alt', 0)}")
